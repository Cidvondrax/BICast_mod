...
public class CheesyShitteryFruitDecorator extends CocoaDecorator {
	public static final CheesyShitteryFruitDecorator INSTANCE = new CheesyShitteryFruitDecorator();
	public static com.mojang.serialization.Codec<CheesyShitteryFruitDecorator> codec;
	public static TreeDecoratorType<?> tdt;
	static {
		codec = com.mojang.serialization.Codec.unit(() -> INSTANCE);
		tdt = new TreeDecoratorType<>(codec);
		tdt.setRegistryName("cheesy_shittery_tree_fruit_decorator");
		ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
	}

	public CheesyShitteryFruitDecorator() {
		super(0.2f);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (random.nextFloat() <= 0.25F) {
						...
						if (Feature.isAir(level, blockpos)) {
							biConsumer.accept(blockpos, Blocks.SPONGE.defaultBlockState());
						}
					}
				}
			});
